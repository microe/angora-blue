...
class InteractiveRecognizer(wx.Frame):
    ...
    def __init__(self, lbphPath, cascadePath, scaleFactor=1.3,
                 minNeighbors=4, minSizeProportional=(0.25, 0.25),
                 flags=cv2.cv.CV_HAAR_SCALE_IMAGE,
                 cameraDeviceID=0, rectColor=(0, 255, 0),
        ...
        self._imageWidth = self._capture.get(
                cv2.cv.CV_CAP_PROP_FRAME_WIDTH)
        self._imageHeight = self._capture.get(
                cv2.cv.CV_CAP_PROP_FRAME_HEIGHT)
        size = (self._imageWidth, self._imageHeight)

        self._currDetectedObject = None

        self._modelPath = lbphPath
        self._recognizer = cv2.createLBPHFaceRecognizer()
        if os.path.isfile(lbphPath):
            self._recognizer.load(lbphPath)
    ...
    def _onCloseWindow(self, event):
        ...
        if self._modelTrained:
            modelDir = os.path.dirname(self._modelPath)
            if not os.path.isdir(modelDir):
                os.makedirs(modelDir)
            self._recognizer.save(self._modelPath)
        self.Destroy()
    ...
    def _updateModel(self, event):
        ...
        labelAsInt = utils.fourCharsToInt(labelAsStr)
    ...
    def _clearModel(self, event=None):
        self._modelTrained = False
        self._clearModelButton.Disable()
        if os.path.isfile(self._modelPath):
    ...
    def _showImage(self, image):
        if image is None:
            ...
            bitmap = utils.wxBitmapFromCvImage(image)
        # Show the bitmap.
    ...
    def _showMessage(self, message):
        self._predictionStaticText.SetLabel(message)